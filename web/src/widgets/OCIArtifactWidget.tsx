...
import { OCIRepository } from "@kubernetes-models/flux-cd/source.toolkit.fluxcd.io/v1beta2";


export type OCIArtifactWidgetProps = {
  source: OCIRepository
}

export function OCIArtifactWidget(props: OCIArtifactWidgetProps) {
  const { source } = props
  ...
  return (
    <>
      ...
      <span className="block field text-neutral-600">{source.spec?.url ?? 'unknown'}</span>
    </>
  )
}
