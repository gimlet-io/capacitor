...
import { RevisionWidget } from './Kustomization.tsx'
...
export interface ServiceProps {
  ...
  handleNavigationSelect: (selectedNav, objectNs, objectName) => void
}

function Service(props: ServiceProps) {
  const { service, kustomization, source, helmRelease, store, handleNavigationSelect } = props;
...
function configMaps(pods, namespace) {
  ...
  return (
    <>
      {configMaps.map(configMap => {
        return <ConfigMap key={configMap} name={configMap} namespace={namespace} />
      })}
    </>
  )
}

function secrets(pods, namespace) {
  let secrets: string[] = []
  ...
  return (
    <>
      {secrets.map(secret => {
        return <Secret key={secret} name={secret} namespace={namespace} />
      })}
    </>
  )
}

function ConfigMap({ name, namespace }) {
...
function Secret({ name, namespace }) {
