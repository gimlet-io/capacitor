(() => {
  ...
  async function fetchPods() {
    try {
      ...
      podListElement.innerHTML = data.items.map((pod) => `
            <div class="pod-item">
      ...
      const podListElement = document.getElementById("podList");
      if (podListElement) {
        podListElement.innerHTML = `<div class="error">Error fetching pods: ${error instanceof Error ? error.message : String(error)}</div>`;
      }
    }
  }
  fetchPods();
